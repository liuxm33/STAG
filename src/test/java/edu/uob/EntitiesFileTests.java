...
final class EntitiesFileTests {
  ...
  void testBasicEntitiesFileIsReadable() {
      try {
          ...
          //被拆解为sections，sections.get(0)是location，sections.get(1)是path

          // The locations will always be in the first subgraph
          ArrayList<Graph> locations = sections.get(0).getSubgraphs();
          Graph firstLocation = locations.get(0);
          //Returns all Nodes of the graph.
          // If the graph contains subgraphs, you can use the parameter to decide
          // if nodes defined in subgraphs should be added to the list also.
          Node locationDetails = firstLocation.getNodes(false).get(0);

          // Yes, you do need to get the ID twice ! 因为cabin是一个类，类的名字叫cabin
          // The first time you get the ID, you get the ID of the Node object
          // The second time you get the ID, you get the Name of the ID object
